...
class Api extends dist.Api {
    ...
    constructor(clientId, secret) {
        super('https://api.fitbit.com', [new dist.DefaultResponseProcessor(FitbitApiException), new ResponseProcessor()]);
    ...
    getLoginUrl(redirectUri, scope, {
        ...
        return `https://www.fitbit.com/oauth2/authorize${dist.Api.convertParametersToUrl(_extends({
    ...
    async requestToken(parameters) {
        const response = await this.request(`oauth2/token${dist.Api.convertParametersToUrl(parameters)}`, 'POST', {}, {
        ...
        return _extends({}, response.data, {
            expireDate: luxon.DateTime.utc().plus({ seconds: response.data.expires_in }).toISO()
        });
    }
    ...
    async getIntradayData(resource, from, to, detail = '1min') {
        ...
        const sets = response.data[`activities-${resource}-intraday`].dataset.map(set$$1 => {
            return {
                time: luxon.DateTime.fromFormat(`${from.toFormat(this.dateFormat)}${set$$1.time}`, `${this.dateFormat}HH:mm:ss`),
                value: set$$1.value
            };
        });
